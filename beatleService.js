...
function readallClickTracksFromUser(req, res, next) {
  ...
  db.oneOrNone('SELECT clickTrack.ID, clickTrack.userID, clickTrack.name, clickTrack.date FROM clickTrack JOIN theUser ON clickTrack.userID = theUser.ID WHERE theUser.ID = $1 ORDER BY clickTrack.name ASC;', [id])
    .then((data) => {
      returnDataOr404(res, data);
    })
    .catch((err) => {
